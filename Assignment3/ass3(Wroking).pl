...
checkHistory(PossesivePronounPropertyList , Refs):-
...
getObject(object(ObjectPassed), Refs):-
	possessive(PossesivePronoun, Thing) = ObjectPassed,
	possessive(PossesivePronoun, PossesivePronounPropertyList),
	checkHistory(PossesivePronounPropertyList, Refs),
	getActor(actor(Thing)).

% ADD ACTOR PERSONAL 
getActor(actor(ActorPassed)):-
	thing(ActorName, _) = ActorPassed,
	thing(ActorName, PropertyList),
	assert(history(thing(ActorName, PropertyList))).

getActor(actor(ActorPassed)):-
	personal(Pronoun, _) = ActorPassed,
	personal(Pronoun, PropertyList),
	assert(history(thing(Pronoun, PropertyList))).

processObject([],[]).
processObject([Head | Tail], Refs):-
	getObject(Head, Refs).

processActor([],[]).
processActor([Head | Tail], Refs):-
	getActor(Head),
	processObject(Tail, Refs).

% You have to write this:
process(LogicalForm, Ref1, Ref2):-
	event(EventName, PropsList) = LogicalForm,
	processActor(PropsList, Ref2),
