...
append([], X, X). 
append([Head | Tail], X, [Head | Tail2]):-
	append(Tail, [X], Tail2).


checkHistory(PossesivePronounPropertyList , Refs, RefsListPassed):-
	member(number(Number), PossesivePronounPropertyList),
	member(gender(Gender), PossesivePronounPropertyList),
	history(thing(Ref, [_, gender(Gender), number(Number)])),
	append(RefsListPassed, [Ref], Refs).
% getObject when there is a possessive noun present.
% for normal sentences such as "John lost his keys"

getObject(object(ObjectPassed), Refs, RefsListPassed):-
	possessive(PossesivePronoun, Thing) = ObjectPassed,
	possessive(PossesivePronoun, PossesivePronounPropertyList),
	checkHistory(PossesivePronounPropertyList, Refs, RefsListPassed),
	getActor(actor(Thing), Refs, RefsListPassed).

getObject(object(ObjectPassed), Refs, RefsListPassed):-
	personal(PersonalPronoun) = ObjectPassed,
	personal(PersonalPronoun, PersonalPronounPropertyList),
	checkHistory(PersonalPronounPropertyList, Refs, RefsListPassed).
	

% ADD ACTOR PERSONAL 
getActor(actor(ActorPassed), _, _):-
	thing(ActorName, _) = ActorPassed,
	thing(ActorName, PropertyList),
	assert(history(thing(ActorName, PropertyList))).

getActor(actor(ActorPassed), Refs, RefsListPassed):-
	personal(Pronoun) = ActorPassed,
	personal(Pronoun, PropertyList),
	checkHistory(PropertyList, Refs, RefsListPassed).

processObject([],_, _).
processObject([Head | Tail], Refs, RefsListPassed):-
	getObject(Head, Refs, RefsListPassed),
	processObject(Tail, Refs, RefsListPassed).

processActor([],[]).
processActor([Head | Tail], Refs, RefsListPassed):-
	getActor(Head, Refs, RefsListPassed),
	processObject(Tail, Refs, RefsListPassed).

% You have to write this:
process(LogicalForm, Ref1, Ref2):-
	event(_, PropsList) = LogicalForm,
	processActor(PropsList, Ref2, Ref1),
	assert(history(LogicalForm)).
